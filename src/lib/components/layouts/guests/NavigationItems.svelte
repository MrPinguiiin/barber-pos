...
<div id="navbar-collapse-with-animation" class="hs-collapse sm:block overflow-hidden transition-all duration-300 basis-full grow">
    <div class="flex flex-col gap-y-4 gap-x-0 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
      <!-- <Button variant="link" href="/">
        Landing
      </Button> -->

      {#if user && authenticated == true && hasCustomerSession == false}
      <Button class="flex items-center gap-x-2 font-medium text-black bg-white hover:bg-gray-300 sm:border-s sm:border-white/[.3] sm:my-6 sm:ps-6" href="/login">
        Dashboard
      </Button>

        <form action="/logout" method="post">
            <Button type="submit" class="flex items-center gap-x-2 font-medium text-black bg-white hover:bg-gray-300 sm:my-6">
      ...
      {:else if user == null && authenticated == false && hasCustomerSession == false}
        ...
        {#if user == null && authenticated == false}
          <Button class="flex items-center gap-x-2 font-medium text-black bg-white hover:bg-gray-300 sm:my-6" href="/sign-in">
            <svg class="flex-shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            Member
          </Button>
        {/if}
        ...
        <form action="/sign-out" method="post" use:enhance>
            <button class="flex items-center gap-x-2 font-medium text-black bg-white hover:bg-gray-300 sm:my-6">
                <svg class="flex-shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                Logout
            </button>
        </form> 
